...
# 4. Control Bus (Signal Layout)

The processor uses a fixed-width control bus generated by the Control Unit for every instruction.

The full layout of the control word is:

```text
[WE_A:1][WE_B:1][SP_WE:1][SRC_A:2][SRC_B:1][SP_INC:1][ALUOP:3][MEM_READ:1][MEM_WRITE:1][MEM_SRC:2][MEM_ADDR_SRC:2][PC_CTL:2][PC_SRC:2]
```

Each instruction is translated into a specific combination of these signals. There is no microcode layer — the opcode is directly mapped to this control word.

### Signal description

| Signal         | Bits | Description                                                                                      |
| -------------- | ---- | ------------------------------------------------------------------------------------------------ |
| `WE_A`         | 1    | Write enable for register A. When set, the selected input is written into A.                     |
| `WE_B`         | 1    | Write enable for register B. When set, the selected input is written into B.                     |
| `SP_WE`        | 1    | Write enable for stack pointer (SP).                                                             |
| `SRC_A`        | 2    | Source for register A: **00** → immediate, **01** → ALU result, **10** → memory                  |
| `SRC_B`        | 1    | Source for register B: **0** → immediate, **1** → memory                                         |
| `SP_INC`       | 1    | Stack pointer direction: **1** → increment, **0** → decrement                                    |
| `ALUOP`        | 3    | Selects ALU operation (ADD, SUB, AND, etc.)                                                      |
| `MEM_READ`     | 1    | Enables memory read                                                                              |
| `MEM_WRITE`    | 1    | Enables memory write                                                                             |
| `MEM_SRC`      | 2    | Value written to memory: **00** → A, **01** → B, **10** → SP                                     |
| `MEM_ADDR_SRC` | 2    | Memory address source: **00** → immediate, **01** → register B, **10** → SP                      |
| `PC_CTL`       | 2    | PC control: **00** → increment, **01** → load if A = 0, **10** → unconditional load              |
| `PC_SRC`       | 2    | Source for new PC: **00** → immediate, **01** → memory, **10** → register A, **11** → register B |


## How this is used

Each instruction corresponds to a specific control word.
...
* `MEM_WRITE = 1`
* `MEM_SRC = 00` (A)
* `MEM_ADDR_SRC = 00` (immediate)
* `PC_CTL = 00`


### `CALL addr`

* `MEM_WRITE = 1` (store return address)
* `MEM_SRC = 00` (PC or computed next PC)
* `MEM_ADDR_SRC = 10` (SP)
* `SP_INC = 1`
* `PC_CTL = 10` (load)
* `PC_SRC = 00` (immediate)
...
* `PC_CTL = 10`
* `PC_SRC = 10` (register A)


## Notes

The control bus is implemented in Logisim as a bundle of wires going from the Control Unit to all major components.

Because the design is direct (opcode → control signals), it is easy to trace what each instruction does by observing the signal pattern in the circuit.
...
# 13. What Is Deliberately Missing
